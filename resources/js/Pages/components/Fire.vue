<template>
<div class="mt-1 fire flex flex-row justify-between">
    <!--<span class="btn" @click="fire"><i class="fas fa-fire"></i></span>-->
    <div class="flex flex-row">
        <span class="btn" @click="fire(postIds.post_id)">
        ...
        <!--<span class="btn"><i class="fas fa-comment-alt"></i></span>-->
        <span class="btn">
            <!--<i class="flip-h far fa-comment"></i>-->
    ...
    <!--<span class="btn"><i class="fas fa-share-alt"></i></span>-->
</div>
</template>
<script>
...
export default {
    ...
    methods: {
        /*
        fire() {
            this.$inertia.put('/post/like/',  this.post);
            this.$emit('fireReload');
        }
        */

        fire(id) {
           axios.put('/api/post/like', this.postIds)
           .then(response => {
               console.log(response.data)
               this.$emit('fireReload', id);
           })
           .catch(error => {
               console.log(error)
           })
...
.fa-comment-alt:hover, .fa-share-alt:hover {
    transition:0.3s ease;
    color:rgb(63, 63, 63);
    cursor: pointer;
}
